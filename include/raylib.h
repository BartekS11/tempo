...
typedef struct Mesh {
    ...
    unsigned char* boneIds; // Vertex bone ids, max 255 bone ids, up to 4 bones influence
...
RLAPI void ToggleBorderlessWindowed(void); // Toggle window state: borderless windowed, resizes
...
int                             height); // Set window minimum dimensions
             // (for FLAG_WINDOW_RESIZABLE)
RLAPI void SetWindowMaxSize(int width,
int                             height); // Set window maximum dimensions
             // (for FLAG_WINDOW_RESIZABLE)
...
RLAPI const char* GetMonitorName(int monitor); // Get the human-readable, UTF-8 encoded name
...
int                             y,
int                             width,
int height);                     // Begin scissor mode (define screen area for
                                 // following drawing)
...
int                                     locIndex,
Texture2D texture);                     // Set shader uniform value and
                                        // bind the texture (sampler2d)
...
RLAPI Ray GetScreenToWorldRayEx(Vector2 position,
Camera                                  camera,
int                                     width,
int height); // Get a ray trace from screen position
             // (i.e mouse) in a viewport
RLAPI Vector2 GetWorldToScreen(Vector3 position,
Camera camera); // Get the screen space position for a 3d
...
Camera2D camera); // Get the world space position for a
...
// control over the frame processing By default EndDrawing() does this job:
...
int                                              dataSize,
const char* fileName); // Export data to code (.h), returns
                       // true on success
...
char* text); // Save text data to file (write), string must be
             // '\0' terminated, returns true on success
//------------------------------------------------------------------
...
const char* filePath); // Get filename string without extension (uses
                       // static string)
RLAPI const char* GetDirectoryPath(const char* filePath); // Get full path for a given fileName with path
                                                          // (uses static string)
RLAPI const char* GetPrevDirectoryPath(
const char* dirPath); // Get previous directory path for a given path
                      // (uses static string)
RLAPI const char* GetWorkingDirectory(void); // Get current working directory (uses static string)
RLAPI const char* GetApplicationDirectory(void); // Get the directory of the running
                                                 // application (uses static string)
RLAPI int MakeDirectory(const char* dirPath); // Create directories (including full
                                              // path requested), returns 0 on success
...
bool scanSubdirs); // Load directory filepaths with extension filtering
                   // and recursive directory scan. Use 'DIR' in the
                   // filter string to include directories in the result
...
int                                                    dataSize,
int* compDataSize); // Compress data (DEFLATE algorithm),
                    // memory must be MemFree()
RLAPI unsigned char* DecompressData(const unsigned char* compData,
int                                                      compDataSize,
int* dataSize); // Decompress data (DEFLATE algorithm),
                // memory must be MemFree()
RLAPI char* EncodeDataBase64(const unsigned char* data,
int                                               dataSize,
int* outputSize); // Encode data to Base64 string, memory
                  // must be MemFree()
...
RLAPI unsigned int*  ComputeMD5(unsigned char* data,
 int dataSize); // Compute MD5 hash code, returns static int[4]
                // (16 bytes)
RLAPI unsigned int* ComputeSHA1(unsigned char* data,
int dataSize); // Compute SHA1 hash code, returns static int[5]
               // (20 bytes)
...
RLAPI void SetAutomationEventBaseFrame(int frame); // Set automation event internal
                                                   // base frame to start recording
...
RLAPI int GetKeyPressed(void); // Get key pressed (keycode), call it multiple times
                               // for keys queued, returns 0 when the queue is empty
RLAPI int GetCharPressed(void); // Get char pressed (unicode), call it multiple times
                                // for chars queued, returns 0 when the queue is empty
RLAPI const char* GetKeyName(
int key); // Get name of a QWERTY key on the current keyboard layout (eg
          // returns string 'q' for KEY_A on an AZERTY keyboard)
...
RLAPI int SetGamepadMappings(const char* mappings); // Set internal gamepad mappings
                                                    // (SDL_GameControllerDB)
RLAPI void SetGamepadVibration(int gamepad,
float                              leftMotor,
float                              rightMotor,
float                              duration); // Set gamepad vibration for both
                 // motors (duration in seconds)
...
RLAPI int GetTouchY(void); // Get touch position Y for touch point 0
                           // (relative to screen size)
RLAPI Vector2 GetTouchPosition(int index); // Get touch position XY for a touch point
...
Rectangle source);                      // Set texture and rectangle to be used
                                        // on shapes drawing
...
int                      posY,
Color                    color); // Draw a pixel using geometry [Can be
              // slow, use with care]
...
RLAPI void DrawLineStrip(const Vector2* points,
int                                     pointCount,
...
Color                                        bottomLeft,
Color                                        topRight,
Color bottomRight); // Draw a gradient-filled rectangle with custom
                    // vertex colors
...
RLAPI void DrawRectangleRoundedLinesEx(Rectangle rec,
float                                            roundness,
int                                              segments,
float                                            lineThick,
...
Vector2                              v2,
Vector2                              v3,
Color                                color); // Draw triangle outline (vertex
              // in counter-clockwise order!)
RLAPI void DrawTriangleFan(const Vector2* points,
int                                       pointCount,
Color color); // Draw a triangle fan defined by points
              // (first vertex is the center)
RLAPI void DrawTriangleStrip(const Vector2* points,
int                                         pointCount,
...
float                              rotation,
float                              lineThick,
Color                              color); // Draw a polygon outline of n
              // sides with extended parameters

// Splines drawing functions
RLAPI void DrawSplineLinear(const Vector2* points,
int                                        pointCount,
float                                      thick,
Color color); // Draw spline: Linear, minimum 2 points
RLAPI void DrawSplineBasis(const Vector2* points,
int                                       pointCount,
float                                     thick,
Color color); // Draw spline: B-Spline, minimum 4 points
RLAPI void DrawSplineCatmullRom(const Vector2* points,
int                                            pointCount,
float                                          thick,
Color color); // Draw spline: Catmull-Rom, minimum 4 points
RLAPI void DrawSplineBezierQuadratic(const Vector2* points,
int                                                 pointCount,
float                                               thick,
Color color); // Draw spline: Quadratic Bezier, minimum 3 points (1
              // control point): [p1, c2, p3, c4...]
RLAPI void DrawSplineBezierCubic(const Vector2* points,
int                                             pointCount,
float                                           thick,
...
RLAPI void DrawSplineSegmentCatmullRom(Vector2 p1,
Vector2                                        p2,
Vector2                                        p3,
Vector2                                        p4,
float                                          thick,
Color color); // Draw spline segment: Catmull-Rom, 4 points
RLAPI void DrawSplineSegmentBezierQuadratic(Vector2 p1,
Vector2                                             c2,
Vector2                                             p3,
float                                               thick,
Color color); // Draw spline segment: Quadratic Bezier, 2 points, 1
              // control point
RLAPI void DrawSplineSegmentBezierCubic(Vector2 p1,
Vector2                                         c2,
Vector2                                         c3,
Vector2                                         p4,
float                                           thick,
...
RLAPI Vector2 GetSplinePointBezierQuad(Vector2 p1,
Vector2                                        c2,
Vector2                                        p3,
...
RLAPI bool CheckCollisionCircleLine(Vector2 center,
float                                       radius,
Vector2                                     p1,
...
int pointCount); // Check if point is within a polygon described by
                 // array of vertices
...
int selectedChannel); // Create an image from a selected channel of
                      // another image (GRAYSCALE)
...
RLAPI void  ImageKernelConvolution(Image* image,
 const float*                             kernel,
...
RLAPI Color* LoadImageColors(Image image); // Load color data from image as
                                           // a Color array (RGBA - 32bit)
RLAPI Color* LoadImagePalette(Image image,
int                                 maxPaletteSize,
int* colorCount); // Load colors palette from image as a
...
RLAPI void ImageDrawLineEx(Image* dst,
Vector2                           start,
Vector2                           end,
int                               thick,
...
RLAPI void ImageDrawTriangle(Image* dst,
Vector2                             v1,
Vector2                             v2,
Vector2                             v3,
Color                               color); // Draw triangle within an image
RLAPI void ImageDrawTriangleEx(Image* dst,
Vector2                               v1,
Vector2                               v2,
Vector2                               v3,
Color                                 c1,
...
Rectangle                   srcRec,
Rectangle                   dstRec,
Color tint); // Draw a source image within a destination image
...
RLAPI bool IsRenderTextureValid(RenderTexture2D target); // Check if a render texture
                                                         // is valid (loaded in GPU)
...
Vector2                                origin,
float                                  rotation,
Color tint); // Draws a texture (or part of it) that
...
float                        alpha); // Get color with alpha applied, alpha
              // goes from 0.0f to 1.0f
RLAPI Color ColorAlphaBlend(Color dst,
Color                             src,
Color tint); // Get src alpha-blended into dst color with tint
RLAPI Color ColorLerp(Color color1,
Color                       color2,
float                       factor); // Get color lerp interpolation between
               // two colors, factor [0.0f..1.0f]
...
int codepointCount); // Load font from file with extended parameters,
                     // use NULL for codepoints and 0 for codepointCount
                     // to load the default character set, font size is
                     // provided in pixels height
...
RLAPI bool       ExportFontAsCode(Font font,
      const char*                      fileName); // Export font as code file, returns
                             // true on success
...
// recommended to store returned data on user-side for re-use WARNING 2:
// Some strings allocate memory internally for the returned strings, those
// strings must be free by user using MemFree()
...
RLAPI char*       TextReplace(const char* text,
      const char*                         replace,
...
RLAPI char*  TextJoin(char** textList,
 int                         count,
 const char*                 delimiter); // Join text strings with delimiter
RLAPI char** TextSplit(const char* text,
char                               delimiter,
...
RLAPI char* TextToPascal(const char* text); // Get Pascal case notation
...
Color color); // Draw a color-filled triangle (vertex in
              // counter-clockwise order!)
RLAPI void DrawTriangleStrip3D(const Vector3* points,
int                                           pointCount,
Color color); // Draw a triangle strip defined by points
...
float                             rotationAngle,
Vector3                           scale,
Color                             tint); // Draw a model wires (with texture
             // if set) with extended parameters
RLAPI void DrawModelPoints(Model model,
Vector3                          position,
float                            scale,
Color                            tint); // Draw a model as points
RLAPI void DrawModelPointsEx(Model model,
Vector3                            position,
Vector3                            rotationAxis,
float                              rotationAngle,
Vector3                            scale,
...
const void*                      data,
int                              dataSize,
int                              offset);                      // Update mesh vertex data in GPU
                                  // for a specific buffer index
...
int instances); // Draw multiple mesh instances with
...
RLAPI void            UpdateModelAnimationBones(Model model,
           ModelAnimation                             anim,
...
Sound source); // Create a new sound that shares the same sample data
               // as the source sound, does not own the sound data
...
RLAPI bool IsAudioStreamValid(AudioStream stream); // Checks if an audio stream is
                                                   // valid (buffers initialized)
RLAPI void UnloadAudioStream(AudioStream stream); // Unload audio stream and free memory
RLAPI void UpdateAudioStream(AudioStream stream,
const void*                              data,
int frameCount); // Update audio stream buffers with data
RLAPI bool IsAudioStreamProcessed(AudioStream stream); // Check if any audio stream
                                                       // buffers requires refill
...
AudioCallback processor); // Attach audio stream processor to the entire
                          // audio pipeline, receives frames x 2 samples
                          // as 'float' (stereo)
RLAPI void DetachAudioMixedProcessor(
AudioCallback processor); // Detach audio stream processor from the
